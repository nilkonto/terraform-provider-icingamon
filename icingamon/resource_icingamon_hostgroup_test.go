...
func TestAccCreateBasicHostGroup(t *testing.T) {
	...
	var testAccCreateBasicHostGroup = fmt.Sprintf(`
		resource "icingamon_hostgroup" "tf-hg-1" {
		...
		Steps: []resource.TestStep{
			resource.TestStep{
				...
				Check: resource.ComposeTestCheckFunc(
					testAccCheckHostgroupExists("icingamon_hostgroup.tf-hg-1"),
					testAccCheckResourceState("icingamon_hostgroup.tf-hg-1", "name", "terraform-hostgroup-1"),
					testAccCheckResourceState("icingamon_hostgroup.tf-hg-1", "display_name", "Terraform Test HostGroup"),
				),
			},
		},
	})
