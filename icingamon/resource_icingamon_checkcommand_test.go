...
func TestAccCreateCheckcommand(t *testing.T) {
	...
	var testAccCreateCheckcommand = fmt.Sprintf(`
		resource "icingamon_checkcommand" "checkcommand" {
		...
		Steps: []resource.TestStep{
			resource.TestStep{
				...
				Check: resource.ComposeTestCheckFunc(
					testAccCheckCheckcommandExists("icingamon_checkcommand.checkcommand"),
					testAccCheckResourceState("icingamon_checkcommand.checkcommand", "name", "terraform-test-checkcommand-1"),
					testAccCheckResourceState("icingamon_checkcommand.checkcommand", "command", "/usr/local/bin/check_command"),
					testAccCheckResourceState("icingamon_checkcommand.checkcommand", "arguments.%", "2"),
					testAccCheckResourceState("icingamon_checkcommand.checkcommand", "arguments.-I", "$IARG$"),
					testAccCheckResourceState("icingamon_checkcommand.checkcommand", "arguments.-J", "$JARG$"),
				),
			},
		},
	})
